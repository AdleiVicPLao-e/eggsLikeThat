...
export const useGameAPI = (options = {}) => {
  ...
  // 🔐 Authentication
  // ---------------------------
  const walletRegister = useCallback(async (walletData) => {
  ...
  // 🥚 Eggs
  // ---------------------------
  const getUserEggs = useCallback(async (filters = {}) => {
  ...
  const purchaseEgg = useCallback(async (purchaseData) => {
  ...
  // 🐾 Pets
  // ---------------------------
  const getUserPets = useCallback(async (filters = {}) => {
    const data = await handleRequest(gameAPI.pets.getUserPets, filters);
    if (updatePets && data?.data?.pets) updatePets(data.data.pets);
    return data;
  }, [handleRequest, updatePets]);

  const upgradePet = useCallback(async (petId, upgradeData) => {
    const data = await handleRequest(gameAPI.pets.upgradePet, petId, upgradeData);
    if (updateUser && data?.data?.user) updateUser(data.data.user);
    if (updatePets && data?.data?.pets) updatePets(data.data.pets);
    return data;
  }, [handleRequest, updateUser, updatePets]);

  const trainPet = useCallback(async (petId, trainData) => {
  ...
  const fusePets = useCallback(async (fusionData) => {
  ...
  const claimDailyReward = useCallback(async () => {
    ...
    if (updateUser && data?.data?.user) updateUser(data.data.user);
    if (updateInventory && data?.data?.inventory) updateInventory(data.data.inventory);
    return data;
  }, [handleRequest, updateUser, updateInventory]);

  const startBattle = useCallback(async (battleData) => {
  ...
  const completeQuest = useCallback(async (questData) => {
  ...
  // 💰 Trading
  // ---------------------------
  const listPet = useCallback(async (listingData) => {
  ...
  const purchasePet = useCallback(async (tradeId) => {
